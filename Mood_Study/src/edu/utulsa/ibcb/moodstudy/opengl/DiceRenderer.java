...
public class DiceRenderer implements Renderer {
	...
	public void recursiveDraw(GL10 gl, SceneNode node) {
		if (node.isSimulated()) {
			DefaultMotionState myMotionState = (DefaultMotionState) node
		...
		for (Mesh object : node.getObjects()) {
			...
			gl.glDrawElements(GL10.GL_TRIANGLES, object.getFaceCount() * 3,
					GL10.GL_UNSIGNED_SHORT, object.getGLIndices());
		}
	...
	public void init(GL10 gl) {
		...
		// gl.glColorMaterial ( GL10.GL_FRONT_AND_BACK,
	...
	private void cameraLookAt(GL10 gl, Camera cam) {
		...
		GLU.gluLookAt(gl, pos.x, pos.y, pos.z, target.x, target.y, target.z, 0,
				1, 0);
	}

	/*
	 * public void suspend() { animator.stop(); }
	 * 
