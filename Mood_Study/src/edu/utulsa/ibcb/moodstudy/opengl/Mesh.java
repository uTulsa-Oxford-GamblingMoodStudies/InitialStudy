...
public class Mesh {
	...
	public void createGLArray() {
		...
		for (short i = 0; i < faces.length / 3; i++) {
			...
			System.arraycopy(vertices[faces[3 * i + 1]], 0, glverts, i * 9 + 3,
					3);
			System.arraycopy(vertices[faces[3 * i + 2]], 0, glverts, i * 9 + 6,
					3);

			System.arraycopy(tvertices[tfaces[3 * i]], 0, gltexcoords, i * 6, 2);
			System.arraycopy(tvertices[tfaces[3 * i + 1]], 0, gltexcoords,
					i * 6 + 2, 2);
			System.arraycopy(tvertices[tfaces[3 * i + 2]], 0, gltexcoords,
					i * 6 + 4, 2);

			System.arraycopy(normals[faces[3 * i]], 0, glnormals, i * 9, 3);
			System.arraycopy(normals[faces[3 * i + 1]], 0, glnormals,
					i * 9 + 3, 3);
			System.arraycopy(normals[faces[3 * i + 2]], 0, glnormals,
					i * 9 + 6, 3);
		...
		while ((line = dataIn.readLine()) != null) {
			...
			if (stack.getLast().equals("*MESH_FACE_LIST")) {
				if (line.startsWith("*MESH_FACE")) {
					...
					Pattern p = Pattern
							.compile("^\\*MESH_FACE ([0-9]+): A: ([0-9]+) B: ([0-9]+) C: ([0-9]+) .*");
