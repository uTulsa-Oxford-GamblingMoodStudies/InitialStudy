...
public class DiceRollEnvironment extends Environment {
	...
	int surfacetex = 0;
	static int target = 1;
	static String[] anim_names = new String[] { "WeakThrow", "MedThrow",
			"StrongThrow" };
	...
	static {
		diefaceOrientations[1][0] = Solver.getRotationMatrix(
				(float) (-Math.PI / 2.0), new Vector3f(0, 1, 0));
		diefaceOrientations[1][1] = new Matrix3f(Solver.identity);
		diefaceOrientations[1][2] = Solver
				.getRotationMatrixZ((float) (Math.PI / 2.0));

		diefaceOrientations[1][3] = Solver.getRotationMatrix((float) (Math.PI),
				new Vector3f(0, 1, 0));
		diefaceOrientations[1][4] = Solver
				.getRotationMatrixZ((float) (-Math.PI / 2.0));
		diefaceOrientations[1][5] = Solver.getRotationMatrix(
				(float) (Math.PI / 2.0), new Vector3f(0, 1, 0));

		diefaceOrientations[0][0] = new Matrix3f(Solver.identity);
		diefaceOrientations[0][1] = Solver.getRotationMatrix(
				(float) (Math.PI / 2.0), new Vector3f(0, 1, 0));
		diefaceOrientations[0][2] = Solver.getRotationMatrix(
				(float) (Math.PI / 2.0), new Vector3f(1, 0, 0));

		diefaceOrientations[0][3] = Solver.getRotationMatrix(
				(float) (-Math.PI / 2.0), new Vector3f(0, 1, 0));
		diefaceOrientations[0][4] = Solver.getRotationMatrix(
				(float) (-Math.PI / 2.0), new Vector3f(1, 0, 0));
		diefaceOrientations[0][5] = Solver.getRotationMatrix((float) (Math.PI),
				new Vector3f(0, 1, 0));

		diefaceOrientations[2][0] = Solver.getRotationMatrix((float) (Math.PI),
				new Vector3f(0, 1, 0));
		diefaceOrientations[2][1] = Solver.getRotationMatrix(
				(float) (-Math.PI / 2.0), new Vector3f(0, 1, 0));
		diefaceOrientations[2][2] = Solver.getRotationMatrix(
				(float) (-Math.PI / 2.0), new Vector3f(1, 0, 0));

		diefaceOrientations[2][3] = Solver.getRotationMatrix(
				(float) (Math.PI / 2.0), new Vector3f(0, 1, 0));
		diefaceOrientations[2][4] = Solver.getRotationMatrix(
				(float) (Math.PI / 2.0), new Vector3f(1, 0, 0));
	...
	public synchronized void initPhysicalProperties() {
		if (!anim_mode)
			die.getRigidBody().applyImpulse(new Vector3f(-throwStrength, 0, 0),
	...
	public synchronized void reloadGLContent(GL10 gl) {
		...
		surfacetex = TextureUtils.loadTexture(gl, DiceRenderer.getContext(),
				R.drawable.table);
	...
	public synchronized void setupEnvironment(GL10 gl) {
		try {
			...
			surfacetex = TextureUtils.loadTexture(gl,
					DiceRenderer.getContext(), R.drawable.table);
			...
			if (isSimulated && !anim_mode) {
				setThrow(20);
				this.buildAnimation("StrongThrow", 10.0f, new FileOutputStream(
						"data\\StrongThrow.anim"));
			} else {
				loadAnimation(DiceRenderer.getContext().getResources()
						.getAssets().open("StrongThrow.anim"));
				loadAnimation(DiceRenderer.getContext().getResources()
						.getAssets().open("MedThrow.anim"));
				loadAnimation(DiceRenderer.getContext().getResources()
						.getAssets().open("WeakThrow.anim"));
			}
	...
	public Mesh createGround() {
		...
		for (int i = 0; i < units + 1; i++) {
			for (int j = 0; j < units + 1; j++) {
				...
				groundMesh.getVertices()[i * (units + 1) + j][0] = facesize * i
						- start;
				groundMesh.getVertices()[i * (units + 1) + j][1] = (float) 0.0;
				groundMesh.getVertices()[i * (units + 1) + j][2] = facesize * j
						- start;
		...
		for (int i = 0; i < units; i++) {
			for (int j = 0; j < units; j++) {
				...
				groundMesh.getFaces()[index] = (short) ((i + 1) * (units + 1)
						+ j + 1);
				groundMesh.getFaces()[index + 1] = (short) ((i + 1)
						* (units + 1) + j);
				...
				groundMesh.getFaces()[index + 5] = (short) ((i + 1)
						* (units + 1) + j + 1);

				groundMesh.getTFaces()[index] = (short) ((i + 1) * (units + 1)
						+ j + 1);
				groundMesh.getTFaces()[index + 1] = (short) ((i + 1)
						* (units + 1) + j);
				groundMesh.getTFaces()[index + 2] = (short) (i * (units + 1) + j);

				groundMesh.getTFaces()[index + 3] = (short) (i * (units + 1) + j);
				groundMesh.getTFaces()[index + 4] = (short) (i * (units + 1)
						+ j + 1);
				groundMesh.getTFaces()[index + 5] = (short) ((i + 1)
						* (units + 1) + j + 1);
